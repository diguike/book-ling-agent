...
export function estimateTokens(text: string): number {
  // 英文/代码约 4 字符/token；CJK 约 1.5 字符/token
...
`字符数 / 4` 是最常见的估算口径，但它只对英文成立。实测（cl100k_base）中文大约 1-1.7 个字符一个 token，直接除以 4 最多会低估 3.5 倍——预算看着还很宽裕，上下文实际早就快满了。所以这里按字符分段：CJK 字符按 1.5 字符/token 算，其余按 4 字符/token 算。这仍然是粗估，中文为主的项目建议把下面的 `reserved` 缓冲再调高一些。精确计算要用 tokenizer（比如 [`tiktoken`](https://github.com/openai/tiktoken)），但那东西很慢，在每次请求前跑一遍不值得。
...
**CLAUDE.md**——等价于我们的 `.ling.md`。Claude Code 从项目根目录和所有父目录自动加载 `CLAUDE.md`。层级越深优先级越高，跟我们一样。不同的是，CLAUDE.md 没有强制截断——无论多长都整篇加载，官方只是建议控制在 200 行以内以保证模型遵循度。"前 200 行、25KB" 的硬截断是下面 Auto Memory 的 MEMORY.md 才有的规则。我们给 `.ling.md` 定的 200 行 / 25KB 限制，借鉴的其实是 Auto Memory 这条规则。

**Auto Memory**——Claude Code 有一个记忆系统，路径在 `~/.claude/projects/<project-slug>/memory/MEMORY.md`（`<project-slug>` 由项目路径派生，斜杠替换成短横线）。用户可以让 Claude "记住"某些偏好（比如"我喜欢用 arrow function"），它会自动写入这个文件，下次启动自动加载。我们的 Ling 目前没做这个，但实现起来不难——就是一个持久化的 key-value 文件。

**Rules 目录**——`<project>/.claude/rules/` 下可以放多个规则文件。每个文件可以通过 frontmatter 的 `paths` 字段指定路径匹配规则（列表形式，值支持 glob），比如 `paths: ["**/*.test.ts"]` 表示只在处理测试文件时加载这条规则。这比一个大文件更灵活，但我们这个阶段用 `.ling.md` 就够了。
