...
**为什么 messages 直接用 OpenAI 的类型？** 因为这就是我们实际传给 API 的数据结构。如果做一层抽象转换，会话恢复时还得转回去，多此一举。如果你要支持多个 LLM 提供商，可以定义一个通用的 Message 类型然后做适配——但在我们当前这个阶段，YAGNI。另外说明一下：本章示例代码为聚焦会话和记忆，退回了单 Provider 的裸 OpenAI client（通过 `OPENAI_BASE_URL` 仍可指向任意兼容端点），多 Provider 实现见第 5、6 章。
...
Sessions:
  ...
  "migration"  12 msgs  2026/5/15 14:30:00
    帮我把数据库从 MySQL 迁移到 PostgreSQL

  e7d9a4b2  6 msgs  2026/5/14 09:15:00
    这个项目的测试覆盖率怎么样
```
...
读出来只是第一步——还要真正拼进发给 API 的 system 内容，LLM 才能看到。做法是在 `main()` 启动时加载一次，拼在基础 system prompt 后面：
...
`agentLoop` 里每轮请求发的都是 `activeSystemPrompt`，所以记忆会跟着 system 消息一起出现在每一轮上下文里：
...
// JSONL（Claude Code 的方案）——每行一条记录，type 直接是 user/assistant
{"type":"summary","summary":"...","leafUuid":"..."}
{"type":"user","sessionId":"xxx","uuid":"...","message":{"role":"user","content":"..."}}
{"type":"assistant","sessionId":"xxx","uuid":"...","message":{"role":"assistant","content":"..."}}
```
...
Claude Code 的 Auto Memory 只有一种记忆类型，按项目隔离：每个项目对应一个记忆目录 `~/.claude/projects/<project-slug>/memory/`，其中 `<project-slug>` 由项目路径派生。如果要类比"用户级 / 项目级"这样的多级作用域，对应的其实是另一套机制——subagent 的持久化 memory（存放在 `~/.claude/agent-memory/<name>/` 等位置），它才分 user / project / local 三种作用域。我们的 user / project / feedback 三分类是 Ling 自己的设计选择，Claude Code 官方并没有这套分类。

和我们一样，Claude Code 也用 `MEMORY.md` 作为索引文件，具体的知识可以拆成独立的 topic 文件放在同一目录下。

一个关键设计：**MEMORY.md 自动加载到上下文时只读前 200 行**。这和我们的 `loadForContext` 一样——记忆是压缩过的知识，不应该占太多上下文窗口。

Claude Code 的记忆写入也由 LLM 自行判断，会话过程中随时读写记忆目录。官方文档没有提到"对话结束时统一做记忆回顾"之类的机制——如果你担心实时判断有遗漏，可以给 Ling 加一步退出前的记忆检查，但这属于自选增强，不是对 Claude Code 的模仿。

### 可以学到什么

1. **JSONL 格式**——如果你的会话经常超过 100 轮，换成 JSONL 会有明显的性能收益
2. **forkSession**——低成本探索不同方案的利器
3. **按项目隔离记忆**——不同项目的记忆分开存放，切换项目时不会带入无关记忆
...
对照 Claude Code 的实现，我们能看到几个进阶方向：JSONL 追加写入、会话分叉、按项目隔离的记忆目录。这些都不难加，但在需要之前不急着加。
